...
export class MovieList extends Component {
    ...
    async componentDidMount() {
        const res = await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=fb0a14ae2f1d06348552ee5d6af1bea6&language=en-US&page=${this.state.currPage}`)
                        ...
                        this.state.movies.map((movieElem) => (
                            <div className="card movie-card" onMouseEnter={() => this.setState({ hover: movieElem.id })} onMouseLeave={() => this.setState({ hover: '' })}>
                                ...
                                <div className='button-wrapper' style={{ display: 'flex', justifyContent: 'center' }}>
                                    ...
                                    {
                                        this.state.hover == movieElem.id && (<a className="btn btn-primary movies-button text-center"
                        ...
                        <ul className="pagination" >
                            <li className="page-item"><a className="page-link" onClick={this.handlePrevious}>Previous</a></li>
                            {
                                this.state.parr.map((value) => (
                                    <li className="page-item"><a className="page-link" onClick={()=> this.handlePageClick(value)}>
                                        {value}
                                        </a>
                                    </li>
                                )
